...
/** The patch stats should be read from: whichever game_version comp_stats
 * was most recently (re)computed for. Returns null if the pipeline hasn't
 * computed any stats yet. */
export async function getCurrentGameVersion(): Promise<string | null> {
  const result = await getPool().query<{ game_version: string }>(
    "SELECT game_version FROM comp_stats ORDER BY updated_at DESC LIMIT 1"
  );
